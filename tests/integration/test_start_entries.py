...
async def test_get_all_start_entries(
    ...
    with aioresponses(passthrough=["http://127.0.0.1"]) as m:
        m.post("http://users.example.com:8081/authorize", status=204)
        resp = await client.get(f'races/{race["id"]}/start-entries', headers=headers)
        assert resp.status == 200
        assert "application/json" in resp.headers[hdrs.CONTENT_TYPE]
...
@pytest.mark.integration
async def test_get_start_entry_by_id_no_authorization(
...
@pytest.mark.integration
async def test_list_start_entries_no_authorization(
    client: _TestClient, mocker: MockFixture, race: dict, start_entry: dict
) -> None:
    """Should return 401 Unauthorized."""
    mocker.patch(
